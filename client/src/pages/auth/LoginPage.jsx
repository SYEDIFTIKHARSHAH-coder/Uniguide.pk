...
export default function LoginPage() {
  ...
  // Detect if the typed email is the single admin (or if we are explicitly on the admin login route)
  const isAdminEmail = isAdminRoute || email.toLowerCase().trim() === SINGLE_ADMIN_EMAIL.toLowerCase().trim();
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          ...
          {isAdminEmail ? (
            <>
              ...
              <p className="text-slate-500 mb-8">Restricted access — UniGuide.pk Admin only.</p>
            </>
          ) : (
            <>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all sm:text-sm bg-slate-50 text-slate-900"
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full flex justify-center items-center py-2.5 px-4 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white transition-all disabled:opacity-70 ${
                isAdminEmail ? "bg-slate-800 hover:bg-slate-900 focus:ring-slate-800" : "bg-blue-600 hover:bg-blue-700 focus:ring-blue-600"
            ...
            >
              {loading ? (
                ...
                <>
                  {isAdminEmail ? <ShieldCheck className="w-4 h-4 mr-2" /> : <LogIn className="w-4 h-4 mr-2" />}
          ...
          {/* Google Sign-in — hidden for admin email */}
          {!isAdminEmail && (
            <div className="mt-6">
              <div className="relative">
          ...
          {/* Sign up link — hidden for admin email */}
          {!isAdminEmail && (
          ...
          {isAdminEmail && (
            <p className="mt-6 text-center text-xs text-slate-400">
              🔒 Restricted admin area — no registration available.
            </p>
          )}
        </div>
      </div>
